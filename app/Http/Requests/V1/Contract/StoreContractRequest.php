...
class StoreContractRequest extends FormRequest
{
    ...
    {
        return [
            'created_contract'                  => 'required|date_format:Y-m-d',
            ...
            'signatures.*.sign_sequence'        => 'required|numeric|between:1,5',
            'signatures.*.phone'                => 'required|string|size:10',
            'signatures.*.email'                => 'required|email:rfc,dns',
            'signatures_follow'                 => 'required|array',
            'signatures_follow.*.name'          => 'required|string',
            'signatures_follow.*.phone'         => 'required|string|size:10',
            'signatures_follow.*.email'         => 'required|email:rfc,dns',
